...
NOMINEE_INFO_FILES = (
    CURATED_DIR / "us_governor_info.json",
    CURATED_DIR / "us_senate_info.json",
)
...
def collect_popover_nominee_slugs() -> set[str]:
    """Primary-winner slugs used only for governor/senate nominee labels."""
    slugs: set[str] = set()
    for path in NOMINEE_INFO_FILES:
        if not path.exists():
            continue
        data = load_json(path)
        for key, info in data.items():
            if key.startswith("_") or not isinstance(info, dict):
                continue
            for slug in (info.get("nominee_slugs") or {}).values():
                if slug:
                    slugs.add(slug)
    return slugs


def build_market_odds(
    markets: list[TrackedMarket],
    today_snapshot: dict,
    today: str,
    errors: list[str],
    *,
    nominee_slugs: set[str] | None = None,
    fetch_missing_nominees: bool = True,
) -> dict:
    """Lean slug → prices file for calendar popovers (no live browser fetch needed)."""
    format_by_slug = {market.slug: market.odds_format for market in markets}
    by_slug: dict[str, dict] = {}

    for market_data in today_snapshot.get("markets", {}).values():
        slug = market_data.get("slug")
        prices = market_data.get("prices")
        if not slug or not isinstance(prices, dict) or not prices:
            continue
        by_slug[slug] = {
            "odds_format": format_by_slug.get(slug, "candidates"),
            "prices": prices,
        }

    pending_nominees = (
        collect_popover_nominee_slugs() if nominee_slugs is None else nominee_slugs
    )
    for slug in sorted(pending_nominees):
        if slug in by_slug:
            continue
        if not fetch_missing_nominees:
            continue
        try:
            prices = fetch_slug_prices(slug)
        except Exception as exc:  # noqa: BLE001 - collect per-slug failures
            errors.append(f"nominee {slug}: {exc}")
            continue
        if not prices:
            continue
        by_slug[slug] = {
    ...
    return {
        "_comment": (
            ...
            "Latest Polymarket prices for calendar popovers, keyed by event slug."
        ),
        "generated_at": today,
        "markets": len(by_slug),
